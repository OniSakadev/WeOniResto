...
const Contact = () => {
  ...
  const scriptURL = 'https://script.google.com/macros/s/AKfycbxoj5MaP5B1egQfLAiKr_R9njwCBrlSV9-5-NXAYuox4SayAEhDU110zy3TUn5sfp9F7A/exec'
    ...
    <section id="contact" className="flex p-32 max-lg:flex-col max-lg:px-4 ">
      <div className="w-1/2 flex flex-col items-center max-lg:items-center max-lg:w-full">
        ...
        <p className="text-center text-gray-400 text-sm px-20 leading-relaxed max-lg:px-2">
          Este es el lugar para proporcionar sugerencias o críticas, para que podamos
           mejor en proporcionar comida con clase
        </p>
      </div>
      <div className="w-1/2 px-10 max-lg:w-full max-lg:mt-5">
        <form name='forms' onSubmit={
          (e) => {sendForm(e)}
        }>
          ...
          <div className="flex flex-col my-3">
            <label>Yu Mel</label>
            <input type="email" className="rounded-md border-gray-300"
            onChange={(e)=> setEmail(e.target.value)}
            />
          </div>

          <div className="flex flex-col my-3">
            <label>Yu Mesej</label>
            <textarea
              cols="50"
              rows="10"
